...
func (repositorio Publicacoes) BuscarPorUsuarioID(ID uint64) ([]modelos.Publicacao, error) {
	linhas, erro := repositorio.db.Query(
...
func (repositorio Publicacoes) Descurtir(publicacaoID uint64) error {
	statement, erro := repositorio.db.Prepare("update publicacoes set curtidas = case when curtidas - 1 < 0 then 0 else curtidas - 1 end where id = ?")
	if erro != nil {
		return erro
	}
	defer statement.Close()
